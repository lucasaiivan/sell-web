...
class Publications {
  ...
  static String getFormatoPrecio(
      {String moneda = "\$", required double value, bool simplified = false}) {
    // var
  ...
  */
  ...
  static String getFechaPublicacion(
      {required DateTime fechaPublicacion, required DateTime fechaActual}) {
    /** 
    Obtiene la fecha de publicación en formato legible para el usuario.
...
class Utils {
  ...
  void getDetailArqueoScreenShot(
      {required BuildContext context,
      required CashRegister cashRegister}) async {
    // widget : ticket
    ...
    var myLongWidget = Builder(builder: (context) {
      return Center(child: Text('CashRegisterDetailView'));
    });
    // controller
  ...
  void getTicketScreenShot(
      {required TicketModel ticketModel, required BuildContext context}) async {
    // widget : ticket
    //var myLongWidget = Builder(builder: (context) {return TicketView(ticket: ticketModel).body(context: context);});
    var myLongWidget = Builder(builder: (context) {
      return Center(child: Text('TicketView'));
    });
    // controller
    final ScreenshotController screenshotController = ScreenshotController();
  ...
  Future<void> createPdfAndShare(
      {required Uint8List data, required String id}) async {
    // description : crea un pdf y lo comparte (soporta web y otras plataformas)
    final pdf = pw.Document();
    pdf.addPage(
      pw.Page(
        build: (pw.Context context) =>
            pw.Center(child: pw.Image(pw.MemoryImage(data))),
