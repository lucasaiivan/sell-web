...
class AdminProfile {
  ...
  String account =
      ""; // el ID de la cuenta administrada por defecto es el ID del usuario quien lo creo
  String email = ''; // email del usuario
  String name = ''; // nombre del usuario (opcaional)
  bool superAdmin =
      false; // Super administrador es el usaurio que creo la cuenta
  bool admin = false; // permiso de administrador
  Timestamp creation = Timestamp.now(); // Fecha en la que se creo la cuenta
  Timestamp lastUpdate =
      Timestamp.now(); // Fecha en la que se actualizo la cuenta
  Map<String, dynamic> startTime =
      {}; // hora de acceso habilitada para el usuario
  ...
  factory AdminProfile.fromDocument(DocumentSnapshot doc) {
    ...
    return AdminProfile(
      ...
      creation:
          data.containsKey("creation") ? doc["creation"] : Timestamp.now(),
      lastUpdate:
          data.containsKey("lastUpdate") ? doc["lastUpdate"] : Timestamp.now(),
  ...
  AdminProfile.fromDocumentSnapshot(
    ...
    historyArqueo =
        data.containsKey('historyArqueo') ? data['historyArqueo'] : false;
    transactions =
        data.containsKey('transactions') ? data['transactions'] : false;
  ...
  bool get hasAccessDay {
    ...
    for (var day in daysOfWeek) {
      if (day.toString().toLowerCase().replaceAll(' ', '') ==
          dayName.toString().toLowerCase().replaceAll(' ', '')) {
        dayAccess = true;
      }
    }
...
class AccountProfile {
  ...
  AccountProfile.fromMap(Map data) {
    ...
    trialStart =
        data.containsKey('trialStart') ? data['trialStart'] : Timestamp.now();
    trialEnd =
        data.containsKey('trialEnd') ? data['trialEnd'] : Timestamp.now();
  }
  Map<String, dynamic> toJson() => {
        "id": id,
